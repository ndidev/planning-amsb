...
class Awesomplete {
  ...
  /**
   * The value input field is separate from the user input field.
   *
   * @type {boolean}
   */
  separate;
  ...
  /**
   * Input field for the user/labels.
   *
   ...
   * @param {boolean}            options.separate         The value input field is separate from the user input field
  ...
  constructor(input, options = {}, callback = () => {}) {
    ...
    // If "separate" is set to true, register the user and value input fields
    if (options.separate) {
      this.input_user = Awesomplete.$(`${selector}_user`, context);
      this.input_value = Awesomplete.$(`${selector}_value`, context);
    }
  ...
  static REPLACE(item) {
    if (this.separate) {
      if (this.multiple) {
        const before_user = this.input_user.value.match(/^.+,\s*|/)[0];
        const before_value = this.input_value.value.match(/^.+,\s*|/)[0];
        if (!this.duplicates) {
          if (
            !RegExp("\\b" + item.value + "\\b").test(this.input_value.value)
          ) {
            this.input_user.value = before_user + item.label + ", ";
            this.input_value.value = before_value + item.value + ",";
          } else {
            this.input_user.value = before_user;
          }
        } else {
          this.input_user.value = before_user + item.label + ", ";
          this.input_value.value = before_value + item.value + ",";
        ...
        this.input_user.value = item.label;
        this.input_value.value = item.value;
      }
    } else {
      if (this.multiple) {
...
class AwesompleteWithTags extends Awesomplete {
  ...
  updateTagStore() {
    ...
    if (this.separate) {
      this.input_value.value = this.tagStore.join(",");
    }
