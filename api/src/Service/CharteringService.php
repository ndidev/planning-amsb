...
class CharteringService
{
    ...
    public function makeCharterLegFromDatabase(array $rawData): CharterLeg
    {
        $leg = (new CharterLeg())
            ...
            ->setCommodity($rawData["commodity"] ?? '')
            ->setQuantity($rawData["quantity"] ?? '')
            ->setComments($rawData["comments"] ?? '');
     ...
     */
    public function getCharters(): Collection
    {
        return $this->charteringRepository->getCharters();
    }
    ...
    {
        return $this->charteringRepository->getCharter($id);
    }
    ...
    {
        $rdv = $this->makeCharterFromFormData($input);

        return $this->charteringRepository->createCharter($rdv);
    }
